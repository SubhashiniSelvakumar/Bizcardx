...
icon = Image.open("D:\Subhashini\Datascience\Tools\images.jpg")
...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background: url("https://files.123freevectors.com/wp-content/original/213841-purple-blank-wave-business-card-background-vector-graphic.jpg");
...
# Create the selectbox with options and styles
selected = st.selectbox("", list(options.keys()), format_func=lambda option: options[option])
...
if selected == "Modify":
    ...
    try :
        with column1:
            ...
            if business_cards:  # Select a card holder to update
                selected_card = st.selectbox("Select a card holder name to update", business_cards)
                mycursor.execute(
        ...
        # DISPLAYING ALL THE INFORMATIONS
                ...
                # Update the information for the selected business card in the database
                   mycursor.execute("UPDATE card_data SET company_name=%s, card_holder=%s, designation=%s, mobile_number=%s, email=%s, website=%s, area=%s, city=%s, state=%s, pin_code=%s WHERE card_holder=%s",
        ...
        with column2:
            ...
            business_cards = [row[0] for row in result]
            if business_cards:
                selected_card = st.selectbox("Select a card holder name to Delete", business_cards)
                st.write(f"### You have selected :green[**{selected_card}'s**] card to delete")
                st.write("#### Proceed to delete this card?")

                if st.button("Yes Delete Business Card"):
                    mycursor.execute("DELETE FROM card_data WHERE card_holder=%s", (selected_card,))
                    mydb.commit()
                    st.success("Business card information deleted from database.")
                else:
                    st.warning("No business cards available for deletion.")
